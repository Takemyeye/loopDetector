...
# Disegnare linee rosse per i loop
for loop in loop_found:
    start_time = loop[0] * segment_length / sr
    end_time = loop[1] * segment_length / sr
    plt.axvline(x=start_time, color='r', linestyle='--', label=f"Loop inizio: {start_time:.2f}s")
    plt.axvline(x=end_time, color='r', linestyle='--', label=f"Loop fine: {end_time:.2f}s")
