...
# Disegnare punti per i loop
for loop in loop_found:
    start_time = loop[0] * segment_length / sr
    end_time = loop[1] * segment_length / sr
    plt.scatter(start_time, 0, color='r', label=f"Loop inizio: {start_time:.2f}s", zorder=5)
    plt.scatter(end_time, 0, color='r', label=f"Loop fine: {end_time:.2f}s", zorder=5)
